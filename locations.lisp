...
Какие могут быть преобразования с т.з реализации?
...
Какие могут быть преобразования с т.з. интерфейса?
1. Макрос
...
*nplm = *non-persistent-locations-memory* , (строка -> КАРТА). Ранее было написано, что вместо строки может быть и другой объект, но я не умыслю, какой ещё объект может быть
...
l/rorl - прочитали объект из исходника. запишем его местоположение
...
get-stream-location-map-delegate - получить или создать делегата для выходного потока, чтобы было возможно строить карту

l/princ - напечатать объект в поток и попытаться записать его location в карту, 
связанную с этим потоком (при этом пытаемся что-то упрощать, а что - я не знаю)
...
(defstruct slo ; откуда взялся данный объект
  ...
  beg ; в исходном тексте. Для лисповых файлов - смещение в байтах. Для Яра - смещение в буквах
  end)

(deftype КАРТА () '(or olm ses slo))
