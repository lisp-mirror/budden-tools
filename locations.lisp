...
Есть разные системы координат в файле: file-position, число литер от начала файла, (строка,колонка).
Из них проще всего получить file-position, но она не всегда полезна. Мы предпочитаем число литер от начала файла, 
т.к. нам оно нужно при преобразованиях текста. Есть два способа получить его:
...
СМ. ТАКЖЕ
=========
 ../../ит/транслятор/навигация-по-исходникам.lisp
...
(defstruct ses ; для данного отрезка может задавать несколько исходников. Эту структуру нужно хранить в контексте destination, например, 
    ; в olm
  (beg nil :type (nullable integer)) ; в результирующем объекте. Для лисповых файлов - смещение в байтах. Для Яра - смещение в буквах. Первая позиция в файле имеет смещение 0
  (end nil :type (nullable integer)) ; Для лисповых файлов - смещение в байтах. Для Яра - смещение в буквах
  (sources nil :type (nullable (cons (or slo olm))))
  simplified ; t, если уже упрощена
  )


(deftype |Система-координат| () "ОЯ::|системы-координат-в-исходном-тексте-и-их-преобразование| 
 Учтите, что в редакторе смещения начинаются от 1 (так было в EMACS и мы этому следуем)" '(member |сико-ПБу| |сико-ПФ|))
...
(defstruct slo ; откуда взялся данный объект cl:cons
  ...
  beg ; в исходном тексте
  end ; в исходном тексте
  )
...
(defun |Сегменты-имени-и-типа| (|Имя-файла|)
  "Не надеемся на то, что лисп правильно разберёт случай а.б.в"
  (perga
   (let |Имя| (pathname-name |Имя-файла|))
   (let |Тип| (pathname-type |Имя-файла|))
   (let |Имя-и-тип| 
     (cond
      ((and |Тип| (not (string= |Тип| "")))
       (str++ |Имя| "." |Тип|))
      (t |Имя|)))
   (budden-tools:split-sequence #\. |Имя-и-тип|)))

(defun |Очистить-карту-исходников-для-файлов-зпт-похожих-на-сгенерированные-из-данного| (|Входной-файл| |Выходная-директория|)
  (perga
   (let |Сегменты-имени-входного-файла| (|Сегменты-имени-и-типа| |Входной-файл|))
   (flet |Обработать-один-файл| (|Имя-сгенерированного-файла| |Содержимое|)
     (declare (ignore |Содержимое|))
     (let |Сегменты| (|Сегменты-имени-и-типа| |Имя-сгенерированного-файла|))
     (let |Расхождение|
       (mismatch |Сегменты| |Сегменты-имени-входного-файла| :test 'string=))
     (let |Сег-вх| (length |Сегменты-имени-входного-файла|))
     (when (and
            (= |Расхождение| (- |Сег-вх| 1))
            (= (length |Сегменты|) (+ |Сег-вх| 2))
            (every 'digit-char-p (elt |Сегменты| |Сег-вх|))
            (equalp (path-to-a-file |Имя-сгенерированного-файла|) |Выходная-директория|))
       ; похож, берём
       (|Удалить-карты-для-генерируемого-файла| |Имя-сгенерированного-файла|)))
   (maphash #'|Обработать-один-файл| *nplf)))
...
;  (declare (optimize speed))
  (when (track-locations)
    (let1 delegate (get-stream-location-map-delegate stream)
      (setf (gethash (pathname (extract-source-filename-from-stream stream))
                     *nplf)
...
(defun l/find-sources-in-file (filename offset &key (strict nil))
  "Дано место в файле, найти места, откуда произрастает данное место. offset (место) должен быть в координатах |сико-ПБу| т.е. в литерах, считая первую литеру файла за 1 (1-based), то же касается карт (и мы это проверяем). При этом мы теряем возможность вызывать эту ф-ю так, чтобы она указывала на исходники лиспа, поскольку там с.к. - это |сико-ПФ| . Вероятно, нужно добавить сюда ещё параметр, указывающий систему координат. Обратная к ней - |Что-порождено-из-этого-места|"
;  (declare (optimize speed))
  (perga
   (let res
     (l/find-sources-in-map (gethash (pathname filename) *nplf) offset :strict strict))
...
;  (declare (optimize speed))
  (etypecase stream
    ...
    (string-stream nil)
    #+sbcl (sb-impl::fd-stream (pathname (sb-impl::fd-stream-file stream)))
    ))
...
(defvar *stream-to-file-position-to-char-position-maps*
  (SWANK-BACKEND:make-weak-key-hash-table :test 'eq)
  "Потоку сопоставляем карту соответствий между file-position и char-position. См комментарий у ensure-file-position-to-char-position-for-stream")

(defun ensure-file-position-to-char-position-for-stream (stream)
  "Этот метод нахождения позиции в файле появился по той причине, что у нас нет возможности поменять класс потока, к-рый использует компилятор лиспа. Поэтому, если нам нужно что-либо делать с данными, вбитыми в файл лиспа, мы вынуждены открывать файл и строить таблицу соответствий. Это требует открывать файл повторно, а кроме того, реализация методом половинного деления неэффективна - можно было построить вектор и искать соответствия за линейное время. Если создание потока находится под нашим контролем, надо использовать ПОТОКИ-ЗПТ-СЧИТАЮЩИЕ-БУКВЫ-СТРОКИ-И-КОЛОНКИ:|Считающий-входной-поток-литер|, к-рый считает буквы по мере чтения. Хотя возможно, что в старом коде ensure-file-position-to-char-position-for-stream используется шире"
  (perga-implementation:perga
    (:lett stream file-stream stream)
...
(defun |Что-порождено-из-этого-места| (|Ссылка-на-источник| |Смещение|)
  "Находит, какие исходные тексты были порождены из данного места. Является обратной к L/FIND-SOURCES-IN-FILE . Медленная, т.к. карты построены для быстрого решения l/find-sources-in-file . Система координат - |сико-ПБу|"
  (perga
   (let |Находки| nil)
   (flet |Искать-в-одной-таблице| (|Ссылка-на-генерированный-текст| |Карта|)
     (_f append |Находки|
         (|Искать-порождённые-исходники-в-одной-карте|
          |Ссылка-на-источник| |Смещение|
          |Ссылка-на-генерированный-текст| |Карта|)))
   (maphash #'|Искать-в-одной-таблице| *nplf)
   |Находки|))

(defconstant |+Эмпирическая-константа-смещения-при-поиске-порождённых-исходников+| 1)

(defun |Искать-порождённые-исходники-в-одной-карте| (|Ссылка-на-источник| |Смещение| |Ссылка-на-генерированный-текст| |Карта|)
  "В системе коорд |сико-ПБу| возвращает список находок в виде (файл начало конец)"
  (perga
   (etypecase |Карта|
     (null nil)
     (slo
      (let |Ссылка-на-источник-в-карте| (slo-source |Карта|))
      (let |Начало-диапазона-исходного-текста-в-карте| (slo-beg |Карта|))
      (when 
          (and
           (equalp |Ссылка-на-источник-в-карте| |Ссылка-на-источник|)
           (<= |Начало-диапазона-исходного-текста-в-карте| |Смещение|)
           (<= |Смещение| (slo-end |Карта|)))
        (|Предупредить-если-система-координат-не-сико-ПБу| |Карта|)
        (list
         (БУКВЫ-И-МЕСТА-В-ФАЙЛЕ:|MAKE-Место-в-исходнике|
                                :|Источник|
                                (БУКВЫ-И-МЕСТА-В-ФАЙЛЕ:|MAKE-Ссылка-на-источник|
                                                       :|Тип| 'БУКВЫ-И-МЕСТА-В-ФАЙЛЕ:|Тсни-файл|
                                                       :|Данное| |Ссылка-на-генерированный-текст|)
                                :|ПБу| 
                                (+ |Смещение|
                                   (- |Начало-диапазона-исходного-текста-в-карте|)
                                   |+Эмпирическая-константа-смещения-при-поиске-порождённых-исходников+|)))))
     (ses
      (let |Находки| nil)
      (let |Исходники| (ses-sources |Карта|))
      (dolist (И |Исходники|)
        (_f append |Находки| (|Искать-порождённые-исходники-в-одной-карте| |Ссылка-на-источник| |Смещение| |Ссылка-на-генерированный-текст| И)))
      |Находки|)
     (olm
      (let |Находки| nil)
      (dolist (И (olm-ses-list |Карта|))
        (_f append |Находки| (|Искать-порождённые-исходники-в-одной-карте| |Ссылка-на-источник| |Смещение| |Ссылка-на-генерированный-текст| И)))
      |Находки|)
     )))

(defun |Предупредить-если-система-координат-не-сико-ПБу| (slo)
  (unless (eq (|SLO-Система-координат| slo) '|сико-ПБу|)
    (warn "В карте ~S система координат - не КАРТЫ-ИСХОДНИКОВ-ТЕЛО::|сико-ПБу| - не могу точно определить все порождённые исходные тексты" slo)))


#| Это не тест, т.к. заточен на конкретное содержимое файла, к-рое записывается в карты гораздо позже
 (def-trivial-test::! |Обратная-к-L/FIND-SOURCES-IN-FILE.1|
                     '(#S(БУКВЫ-И-МЕСТА-В-ФАЙЛЕ:|Ссылка-на-источник|
                          :|Тип| БУКВЫ-И-МЕСТА-В-ФАЙЛЕ:|Тсни-файл|
                          :|Данное| #p"c:/yar/ит/парсер/тесты/а.5.ярс.lisp")
                         1188 1188 КАРТЫ-ИСХОДНИКОВ-ТЕЛО::|сико-ПБу|)
  (|Что-порождено-из-этого-места| #P"c:/yar/ит/парсер/тесты/а.ярс" 499)
  :test 'equalp)
|#
#|
  0: (КАРТЫ-ИСХОДНИКОВ-ЛИЦО:L/FIND-SOURCES-IN-FILE
      "c:/yar/ит/парсер/тесты/а.5.ярс.lisp" 1188 :STRICT T)
  0: КАРТЫ-ИСХОДНИКОВ-ЛИЦО:L/FIND-SOURCES-IN-FILE returned
       ((#S(БУКВЫ-И-МЕСТА-В-ФАЙЛЕ:Ссылка-на-источник
            :Тип БУКВЫ-И-МЕСТА-В-ФАЙЛЕ:Тсни-файл
            :Данное #P"c:/yar/ит/парсер/тесты/а.ярс")
         499 499 КАРТЫ-ИСХОДНИКОВ-ТЕЛО::сико-ПБу))
|#
