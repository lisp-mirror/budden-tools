...
(defmacro |Добавить-в-точку-роста| (|Значение| |Переменная-результат| |Переменная-точка-роста|)
  "Значение - это значение для добавления (вычисляется 1 раз), Переменная-результа и Переменная-точка-роста - два символа, именующих переменные (не вычисляются). Вроде они могут быть и местами, но будут вычисляться дважды, что не очень эффективно. Также эта конструкция может работать как очередь. Извлечение из очереди - с помощью обычного pop. См. также kons:|BUDDEN-TOOLS-Добавить-в-точку-роста| "
  `(cond
