...
(defmacro collect-into-tail-of-list (|Значение| |Переменная-результат| |Переменная-точка-роста|)
  "Значение - это значение для добавления (вычисляется 1 раз), Переменная-результа и Переменная-точка-роста - два символа, именующих переменные (не вычисляются). Вроде они могут быть и местами, но будут вычисляться дважды, что не очень эффективно. Также эта конструкция может работать как очередь. Извлечение из очереди - с помощью обычного pop. См. также kons:budden-tools-collect-into-tail-of-list "
  `(cond
