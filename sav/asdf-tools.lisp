...
#+ignore (eval-when (:compile-toplevel)
  (error "load this file as a source, don't compile it!"))

(defun loadsys (&rest args) "Shortcut for (asdf:oos 'asdf:load-op ,@args)"
  (apply 'oos 'load-op args))

(defun lo (&rest args) "One more shortcut for (asdf:oos 'asdf:load-op ,@args)"
       (apply 'oos 'load-op args))
...
#+ignore (defmethod perform ((o stat-op) (c cl-source-file))
  (dolist (f (input-files o c))
    (account-for-file o c f)))

#+ignore (defmethod perform ((o stat-op) (c static-file))
  (dolist (f (input-files o c))
    (account-for-file o c f)))
