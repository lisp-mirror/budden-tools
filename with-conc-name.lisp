...
(defmacro with-conc-name (var conc-name &body body)
  "“еперь оно пытаетс€ игнорировать регистр символов "
  (let* ((svar (string var))
         (len (length svar))
         (string-concname (string conc-name)))
   (labels 
             ...
             (let* ((sname (string symbol))
               ...
               (and 
                ...
                (let ((accessor-name
                        ...
                        *package*
                        *readtable*
                        nil ; хм? FIXME разобратьс€, что значит этот nil. 
                        )))
                  (when accessor-name                                                       
                    (values accessor-name var))
                  ))))))
...
(defmacro let-with-conc-type (var type value &body body)
  ...
  (assert (typep type 'symbol) () "let-with-conc-type: ожидали им€ типа, получили ~S" type)
  `(let ((,var ,value))
...
; FIXME - тест требует see-packages, но они ещЄ не загружены
