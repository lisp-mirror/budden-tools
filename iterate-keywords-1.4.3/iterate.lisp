...
;;;  FIXES.
;;; (v. 1.2.1-ansi)
;;;  2008-11-07 - Budden: now you can optionnnaly use keywords for
;;;               clauses, (:for ) instead of (for ). 
;;;               Tested in sbcl-1.0.20/linux-x86-32 only. 
...
(defconst version "1.4.3" "Current version of Iterate-keywords")
...
Evaluate (iterk:display-iterate-clauses) for an overview of clauses"
  (let* ((*env* env)
...
(defun add-loop-body-wrapper (wrapper)
  (pushnew wrapper *loop-body-wrappers* :test #'equalp) ; Budden 08-11-06. As we're
     ; adding a keyword macrolets for finish and the like, we might create several (unnecessary)
     ; identical wrappers. To avoid this, we use pushnew
 ; (push wrapper *loop-body-wrappers* ) ; commented out by Budden 08-11-06
)
...
(defun walk-special-form (form)
  (let ((*clause* form)
	(func (cdr (assoc (if (keywordp (car form))
                              (find-symbol (string (car form)) :iterate-keywords)
                              (car form))
                          *special-form-alist*))))
...
(defun apply-clause-function (func args)
  (when (keywordp func) (setf func (find-symbol (string func) :iterate-keywords)))
  (let ((*initial* nil)
	(*decls* nil)
	(*step* nil)
	(*final* nil)
...
(defun display-iterate-clauses (&optional clause-spec)
  (let ((*package* (find-package '#:iterate-keywords)))
    (fresh-line)
...
(defun install-special-clause-function (symbol &optional doc-string)
  ...
  (let* ((key-symbol (keywordize symbol))
...
(eval-when (:load-toplevel :execute)
  (dolist (x (cdr *clause-info-index*))
    (let ((s (car x)))
      (unless (keywordp s)
	(eval `(defsynonym ,(keywordize s) ,s))))))
